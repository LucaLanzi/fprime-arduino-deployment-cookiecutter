...
// ----------------------------------------------------------------------
// Every size/buffer/handle constant this file used to `#define` below is
// ALSO declared as an FPP `constant` in F' core
// (lib/fprime/default/config/{FpConstants,PlatformCfg}.fpp), autocoded into
// a real enum pulled in by Fw/FPrimeBasicTypes.hpp - so defining it here too
// makes the preprocessor blindly substitute this header's value into the
// enum's own initializer wherever both headers land in the same translation
// unit (e.g. "enum { FW_COM_BUFFER_MAX_SIZE = 512 }" becomes
// "enum { 128 = 512 }"), a hard syntax error. An #ifndef guard can't fix
// this either: an enumerator isn't a macro, so it's invisible to #ifndef
// regardless of include order. Confirmed against F' v4.1.1 and v4.3.0.
//
// All of them are simply removed below rather than re-added as project
// overrides. If a project needs a specific value to differ from the
// framework default, override it via a project FpConstants.fpp/
// PlatformCfg.fpp CONFIGURATION_OVERRIDES (see this deployment's
// config/CMakeLists.txt), not by re-adding a macro here.
// ----------------------------------------------------------------------
